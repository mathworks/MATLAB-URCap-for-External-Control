...
public class RequestProgram {
    ...
    public String sendCommand(String command) {
        ...
        try {
            ...
            result = String.format("popup(\"The connection to the remote PC at " + this.hostIp + ":"
                    + this.portNr + " could not be established. Reason: %s\","
    ...
    public void splitProgram(String program) {
        ...
        for (int it = 0; it < programArray.length; ++it) {
            ...
            } else if (programArray[it].matches("(\\p{Zs}*)# HEADER_END(\\p{Zs}*)")
                && headerBeginFound) {
                headerEndFound = true;
